...
class ExerciseAdapter(private val context: Context, private val viewModel: MainViewModel) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (exercise.timed) {
            ...
            // CRITICAL FIX: Remove listener before updating state to prevent infinite loops
            checkbox.setOnCheckedChangeListener(null)
            checkbox.isChecked = exercise.done
            
            // Add listener after state is set
            checkbox.setOnCheckedChangeListener { _, isChecked ->
                if (isChecked != exercise.done) {  // Only trigger if actually changed
