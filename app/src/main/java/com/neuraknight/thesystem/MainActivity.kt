...
class MainActivity : AppCompatActivity() {
    ...
    private fun showMainScreen() {
        ...
        val profileImage = view.findViewById<ImageView>(R.id.mainProfileImage)
        val userName = view.findViewById<TextView>(R.id.mainUserName)
        val userLevel = view.findViewById<TextView>(R.id.mainUserLevel)
        val userRank = view.findViewById<TextView>(R.id.mainUserRank)
        val userClass = view.findViewById<TextView>(R.id.mainUserClass)
        val userTitle = view.findViewById<TextView>(R.id.mainUserTitle)
        val userPasscards = view.findViewById<TextView>(R.id.mainUserPasscards)
        val xpProgress = view.findViewById<ProgressBar>(R.id.xpProgress)
        val xpText = view.findViewById<TextView>(R.id.xpText)

        // Load profile image
        val imgPath = viewModel.appData.user.profileImg
        if (imgPath.isNotEmpty()) {
            try {
                val bitmap = android.graphics.BitmapFactory.decodeFile(imgPath)
        ...
        // Set profile text
        userName.text = viewModel.appData.user.name.ifEmpty { "Unknown" }
        userLevel.text = "Lvl ${viewModel.appData.user.level}"
        userRank.text = "[${viewModel.appData.user.rank}]"
        userClass.text = viewModel.appData.user.characterClass
        userTitle.text = viewModel.appData.user.currentTitle
        userPasscards.text = "🎫 ${viewModel.appData.user.passcards}"

        // Set XP progress
        val progress = (viewModel.appData.user.xpProgress / viewModel.appData.user.xpNeeded * 100).toInt()
        xpProgress.progress = progress
        xpText.text = "${viewModel.appData.user.xpProgress.toInt()}/${viewModel.appData.user.xpNeeded.toInt()}"
